...
const setup = async (req, res) => {
  ...
  for (const filePath of settingsFiles) {
    ...
    const settingsToUpdate = {
      bizflow_app_email: email,
      bizflow_app_company_email: email,
      bizflow_app_timezone: timezone,
      bizflow_app_country: country,
      bizflow_app_language: language || 'en_us',
    };
  ...
  return res.status(200).json({
    success: true,
    result: {},
    message: 'Successfully bizflow App Setup',
  });
};

...
const forgetPassword = async (req, res, { userModel }) => {
  ...
  const bizflow_app_email = settings['bizflow_app_email'];
  const bizflow_base_url = settings['bizflow_base_url'];

  const url = checkAndCorrectURL(bizflow_base_url);
  ...
  await sendMail({
    ...
    name: user.name,
    link,
    subject: 'Reset your password | BizFlow',
    bizflow_app_email,
